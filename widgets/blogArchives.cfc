...
component extends="contentbox.models.ui.BaseWidget" singleton{
	...
	blogArchives function init(){
		// Widget Properties
		setName( "blogArchives" );
		setVersion( "1.0" );
	...
	*/
	...
	private function buildList(archives,showPostCount){
		...
		saveContent variable="rString"{
			writeOutput('<ul class="pl-0" id="archives">');
			// iterate and create
			for(var x=1; x lte arrayLen( arguments.archives ); x++){
				var thisDate = arguments.archives[ x ]["year"] & "-" & arguments.archives[ x ]["month"] & "-1";
				writeOutput('<li class="archives"><a href="#cb.linkArchive(year=arguments.archives[ x ]['year'],month=arguments.archives[ x ]['month'])#"><i class="fas fa-calendar"></i> #dateFormat(thisDate,"mmmm yyyy" )#');
				if( arguments.showPostCount ){ writeOutput( " (#arguments.archives[ x ]['count']#)" ); }
				writeOutput('</a></li>');
			}
			// close ul
